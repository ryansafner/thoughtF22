...
## [Ricardian Distribution Theory](#5)

## [The Bullion Controversy](#28)

## [The Debate over “General Gluts” and Say’s Law](#36)
...
- If he had applied it *beyond* land to *all factors*, and considered land as having an opportunity cost, we would essentially arrive at the .hi[marginal productivity theory]

- .hi-purple[Rent] is the excess of the product over the marginal farmer paying costs to labor and capital
...
- Rent comes from scarcity of some fixed factor, and differential productivities of its units
...
- As *homogenous* land, being fixed or scarce, creates scarcity rent: the difference between the product of all capital and labor and the product of the final dose at the intensive margin
...
- In reality, land *does* have a supply price that brings new land to market
  - Paying opportunity cost of using land and taking it out of alternative uses (grazing, parking lot, etc)
]
]
...
- Landlords contribute nothing to production, are pure parasites
  - earn rents merely for *possessing* and licensing out a needed factor of production without providing anything socially useful

- Ricardo critical of landlords’s spending on consumption, luxuries, servants
  - viewed as keeping critical resources out of production (production that increases economic growth!)
...
- Again, imagine the economy is one giant firm (farm) that grows 🌾🌾
...
base<-ggplot(data = tibble(x = 1:10))+
  ...
  geom_label(x = 9, y = mp(9), color = "blue", label = expression(MP[LK]))+
...
- Total product = sum of marginal products (area under $MP_{LK}$ curve)
]
...
base+
  ...
  geom_rect(xmin = 0, xmax = 5, ymin = 0, ymax = 5, fill = "blue", alpha = 0.05)+
  geom_text(x = 2.5, y = 2.5, label = "TP(x)", color = "white")
...
- Total product = .orange[average product] $\times$ quantity of doses $(x)$
...
- Rent $=$ difference between .orange[average product] and .blue[marginal product]
]
]
...
.pull-left[
.smallest[
- Banking & currency controversy in early 19<sup>th</sup> C.
...
.pull-right[
.center[
![:scale 60%](../images/bullionistcontroversy.jpg)
...
- War ends in 1815, 1816 was “The Year Without a Summer” — massive crop failures, high prices of Corn
  - possibly a climatic event (1815 volcanic eruption of Mt Tambora)
...
.pull-left[
.smallest[
- Sales *in general can* be dull with an **excess demand for money**
...
pp1 <- ggplot(data = tibble(x=1:10),aes(x=x))+
  ...
  scale_y_continuous(breaks = c(1),
                     labels = c(expression(PPM[1])),
                     limits=c(0,5),
                     expand=c(0,0))+
  theme_classic(base_family = "Fira Sans Condensed", base_size = 14)

pp1 +   annotate("segment", x = 2, xend = 4, y = 1, yend = 1, color = "purple", size=2, alpha=1, arrow=arrow(length=unit(0.5,"cm"), ends="both", type="closed"))+
...
- Decreased spending depresses sales, lowers price level (P), but raises purchasing power, i.e. .hi-purple[deflation]
...
```{r}
pp1+
  annotate("segment", x = 3.75, xend = 2.25, y = 1.25, yend = 2, color = "purple", size=2, alpha=1, arrow=arrow(length=unit(0.5,"cm"), ends="last", type="closed"))
```
]
...
- Adjustment to new equilibrium with lower price level, higher PPM
...
pp1+
  ...
  scale_x_continuous(breaks=c(2),
                     labels=c(expression(q[S]==q[D])),
                     limits=c(0,5),
                     expand=c(0,0))+
  scale_y_continuous(breaks = c(1,2),
                     labels = c(expression(PPM[1]), expression(PPM[2])),
                     limits=c(0,5),
                     expand=c(0,0))
...
.pull-right[
.smallest[
- Ricardo clearly “won” the debate, but a mere band-aid for classical theory
  - Latched onto simplified version, additional assumptions
  - but his position was more logically consistent and impressive

- Malthus had no alternative theory to better articulate his opposition 
